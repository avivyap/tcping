		...
		if [[ $port =~ $regex_port ]]; then
			...
			for i in {1..3};do ### seguir configurando el bucle para hacer varios tcping
				...
				if timeout 2 bash -c "</dev/tcp/$ip/$port" 2>/dev/null;then
					...
					echo -e "\n${greenColour}[+]${endColour}${purpleColour} Host accessible on port $port seq=$i ${endColour}\n"
				else

					echo -e "\n${redColour}[!]${endColour}${purpleColour} Host reachable on port $port seq=$i ${endColour}\n"
